import splashImage from "../img/sushi.jpeg";

const Home = () => {
  return (
    <section className="home">
      <div className="description-card">
        <div className="description-card__text flow">
          <h1>Looking for a place to put your recipes?</h1>
          <p>
            Look no further! <b>RECIPE APP</b> is the app for you!
          </p>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Neque
            soluta in assumenda numquam enim. Sed tempore, accusantium,
            blanditiis itaque unde, corrupti in perferendis asperiores voluptas
            tempora ut aspernatur labore veritatis?
          </p>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati
            quidem nesciunt nihil quisquam dolorem ea?
          </p>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolore
            provident qui alias, impedit aliquam rerum hic tempora totam, neque,
            ipsa debitis ipsum. Iure dolore soluta minima in, rerum adipisci ea
            aperiam. Mollitia libero temporibus atque alias dignissimos cumque
            officiis, fugit qui ipsa! Quod eligendi molestiae, fugit, eos
            obcaecati, eius dolorum explicabo cumque itaque quisquam
            repellendus.
          </p>
        </div>
        <div className="description-card__image">
          <img src={splashImage} alt="Delicious Sushi" />
        </div>
      </div>
    </section>
  );
};

export default Home;
